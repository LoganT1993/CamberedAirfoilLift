...
def LiftNACA(c, m, p, alpha, rho, V):
    ...
    # c = chord length
    # M = first digit, so 5 is 0.05. Max camber
    # P = 2nd digit, % of distance to edge (for 4, is 40% or 0.4). Max camber position
    # alpha = angle of attack, degrees

    c = float(c) ## the program messes up if it gets an int instead of a float for the value of c
    x = sym.Symbol('x')
    z = sym.Symbol('z')
    th = sym.Symbol('theta') #theta
...
cLength = 1.00 # chord length in meters
Vinf = 10 #m/s
rho = 1.23 #kg/m^3
alpha = 3
